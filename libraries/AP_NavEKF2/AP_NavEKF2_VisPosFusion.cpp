...
void NavEKF2_core::SelectVisPosFusion()
{
    ...
    if (visPosDataToFuse && PV_AidingMode == AID_VISPOS)
    {
        // Set the visPos noise used by the fusion processes
        R_LPOS = 0.1f;//sq(MAX(frontend->_visPosNoise, 0.001f));
...
void NavEKF2_core::FuseVisPos()
{
    ...
    for (int8_t obsIndex=0; obsIndex<=1; obsIndex++) { // fuse X axis data first
        ...
        Vector3f pos_bf = Tnb_vispos*stateStruct.position;
        lpos[0] = pos_bf.x;
        lpos[1] = pos_bf.y;
        if(core_index ==0)
        ...
        if (obsIndex == 0) {
            ...
            float t56 = P[8][8]*t24;
            float t57 = P[1][8]*t26;
            float t37 = t34+t35+t36-t56-t57;
            float t38 = P[6][1]*t11;
            float t39 = P[7][1]*t10;
            float t40 = P[2][1]*t23;
            float t59 = P[8][1]*t24;
            float t60 = P[1][1]*t26;
            float t41 = t38+t39+t40-t59-t60;
            float t42 = P[6][2]*t11;
            float t43 = P[7][2]*t10;
            float t44 = P[2][2]*t23;
            float t62 = P[8][2]*t24;
            ...
            float t58 = t24*t37;
            float t61 = t26*t41;
            float t52 = R_LPOS+t33+t46+t51-t58-t61;
            float t53 = 1.0f/t52;
            ...
            Kfusion[0] = t53*(P[0][6]*t11+P[0][7]*t10+P[0][2]*t23-P[0][8]*(t6-q1*q3*2.0f)-P[0][1]*(t14+t17-pe*(t7-q2*q3*2.0f)));
            Kfusion[1] = t53*(P[1][6]*t11+P[1][7]*t10+P[1][2]*t23-P[1][1]*t26-P[1][8]*t24);
            ...
            Kfusion[8] = t53*(-t56+P[8][6]*t11+P[8][7]*t10+P[8][2]*t23-P[8][1]*t26);
            ...
            H_LPOS[0] = pn*(t6+t7)+pd*(t2-t3-t4+t5)-pe*(t10-q2*q3*2.0f);
            ...
            Kfusion[0] = t53*(t29+P[0][7]*t12+P[0][8]*t14-P[0][2]*t26-P[0][6]*(t10-q1*q2*2.0f));
        ...
        if (visPosTestRatio[obsIndex] < 1.0f) {
            ...
            if (healthyFusion) {
                ...
                hal.console->printf("Kfusion ");
                for (uint8_t j= 0; j<=stateIndexLim; j++) {
                    hal.console->printf("%f ", Kfusion[j]);
        ...
        if(core_index == 0)   
            hal.console->printf(" Data After: %f %f\n", stateStruct.position.x, stateStruct.position.y);
